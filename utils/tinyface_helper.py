'''
    Code borrowed from https://github.com/mk-minchul/adaface
'''
...
class TinyFaceTest:
    def __init__(self, tinyface_root='data/tinyface', alignment_dir_name='align'):

        self.tinyface_root = tinyface_root
        # as defined by tinyface protocol
        self.gallery_dict = scipy.io.loadmat(os.path.join(tinyface_root, 'Testing_Set/gallery_match_img_ID_pairs.mat'))
        self.probe_dict = scipy.io.loadmat(os.path.join(tinyface_root, 'Testing_Set/probe_img_ID_pairs.mat'))
        ...
        self.probe_paths = get_all_files(os.path.join(tinyface_root, 'Testing_Set/Probe'))
        self.probe_paths = np.array(self.probe_paths).astype(object).flatten()

        self.gallery_paths = get_all_files(os.path.join(tinyface_root, 'Testing_Set/Gallery_Match'))
        self.gallery_paths = np.array(self.gallery_paths).astype(object).flatten()

        self.distractor_paths = get_all_files(os.path.join(tinyface_root, 'Testing_Set/Gallery_Distractor'))
