...
python3 src/main.py --train --step 1 --setup finetune --arch 50  --test_dataset cpl_fw --saved_model_path arcface_50_ms1m_step0.pth
"""
...
def parse_arguments(argv):
    ...
    parser.add_argument("--test_dataset",  type=str,   default="cpl_fw",   help="Name of the test datasets")
    parser.add_argument('--batch_size',    type=int,   default=256,        help='Batch size') #256
    parser.add_argument('--max_epoch',     type=int,   default=1,          help='Maximum epochs') #20
    ...
    parser.add_argument('--saved_model_path',   type=str,   default="arcface_50_ms1m_step0.pth", help='model pretrained on base/previous step data')
    ...
    if args.train == True:
        if args.is_base == False:
            for i in range(1, args.step_size + 1):
                print("\n\n ******************* Step %d ******************* " % i)
                args.step = i
                args.saved_model_path = "l_%s_%s_ms1m_step%d.pth" % (args.model_type, args.arch, (i - 1))
